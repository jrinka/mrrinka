...
const paper2 = "Paper 2 is the same comparative literary task in both courses. The current rubric has five criteria, A, B1, B2, C and D, each worth five marks (25 total); do not award or predict marks in this coaching tool. Keep the student's chosen question central and use two literary works by different authors. Distinguish two jobs: analyze and evaluate how choices in each work shape meaning (B1), then analyze a meaningful similarity and/or difference between the works (B2). Both similarity and difference are not required. Because the exam is closed-book, precise references can support an idea without verbatim quotation. A list of parallels, plot summary, or two disconnected mini-essays does not establish a developed comparison.";
