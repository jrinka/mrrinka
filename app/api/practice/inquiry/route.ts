...
export async function POST(request:Request){
  ...
  try{
    return Response.json({...await inquiryFeedback(parsed.data),model,policyVersion:"2026-09-21-inquiry-v2",createdAt:new Date().toISOString()},{headers:{"Cache-Control":"no-store"}});
  }catch(error){
