...
test("task-specific coaching is reviewed before release",async()=>{
  ...
  assert.equal(result.refused,false);assert.match(prompts[1],/BOTH works/);assert.match(prompts[1],/B1.*B2/);assert.match(prompts[1],/verbatim quotation/);assert.match(prompts[1],/check whether more instances exist/);assert.match(prompts[2],/unsupplied textual patterns/);assert.deepEqual(!result.refused && result.feedback,feedback);
});
