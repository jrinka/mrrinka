...
import { cyclingExample as example } from "@/lib/cycling-example";
const reading: Record<string, {label:string;phase:string;view:number}> = example.sections;
const fields = [
 {key:"audience",label:"The policy argument",hint:"What relationship connects feasible journeys, projected benefits and policy action?"},
 {key:"evidence",label:"Connected words and images",hint:"Connect a precise figure or phrase with a chart, map or policy icon."},
 {key:"analysis",label:"Your analytical paragraph",hint:"Explain the comparison and its contribution to the argument, keeping the scope of the figures accurate."},
] as const;
import styles from "./elephant-appeal.module.css";

export default function CyclingGuide({ body, href }: { body: string; href: string }) {
  ...
  function goTo(index: number) {
    ...
    const url = `${href}?view=example&example=cycling#${sections[index].id}`;
  ...
  return <div className="advertisement-guide">
    ...
    <div className="advertisement-reading-workspace" ref={workspaceRef}>
      <aside className={`advertisement-viewer ${styles.source}`} ref={sourceRef} tabIndex={0} aria-label="Cycling infographic source views">
        <div className="text-type-example-head"><span className="mono">SOURCE / ITDP</span><button type="button" onClick={() => dialogRef.current?.showModal()}>Enlarge source ↗</button></div>
      ...
      <div className="advertisement-analysis">
        ...
        {sections.map((section, index) => <section className="advertisement-reading-section" key={section.id} hidden={activeIndex !== index} aria-labelledby={section.id}>
          ...
          {section.id === "practice-and-transfer" && <CyclingNotes />}
        </section>)}
    ...
    <dialog ref={dialogRef} className={`advertisement-source-dialog ${styles.dialog}`} aria-labelledby="cycling-source-title" onClick={event => { if (event.target === dialogRef.current) dialogRef.current?.close(); }}>
      <div className="advertisement-dialog-heading"><h2 id="cycling-source-title">Cycling — source viewer</h2><button type="button" autoFocus onClick={() => dialogRef.current?.close()}>Close ×</button></div>
...
function CyclingNotes() {
  ...
  return <section className="infographic-notes" aria-label="Cycling infographic analysis notebook">
    ...
    <div className="tool-actions"><ExportFormatSelect value={format} onChange={setFormat} /><button className="button secondary" type="button" onClick={() => downloadRecord(`Cycling infographic analysis notes\n\nSource: ${example.title}\nhttps://mrrinka.com${example.views[0].image}\nhttps://mrrinka.com${example.views[1].image}\n\nQuestion from the supplied paper: ${example.question}\n\n${fields.map(field => `${field.label}\n${values[field.key] || "(Not yet written)"}`).join("\n\n")}\n\nStudent-authored notes; no AI feedback.`, "cycling-infographic-notes.txt", format)}>Export my notes</button></div>
    <details className="infographic-self-check"><summary>Review your paragraph</summary><ul><li>Have I connected specific wording with the visual comparison?</li><li>Have I distinguished cycling’s contribution from the whole scenario?</li><li>Have I preserved the time frame and units?</li><li>Have I treated projections as conditional rather than achieved outcomes?</li></ul></details>
  </section>;
}
...
function SourceFigure({viewIndex}:{viewIndex:number}) {
 ...
 return <figure className={styles.figure}><Image src={view.image} alt={view.alt} width={view.width} height={view.height} unoptimized /><figcaption>{view.label} · ITDP, The Benefits of Shifting to Cycling. Supplied exam adaptation; UC Davis Institute of Transportation Studies identity also appears in the source. <a href={view.image} target="_blank" rel="noopener noreferrer">Full-size source image ↗</a></figcaption></figure>;
}
