...
export default function TextTypeGuide({ body, itemId, href, showExample }: { body: string; itemId: string; href: string; showExample: boolean }) {
  ...
  return <>
    ...
    {showExample && worked ? <>
      ...
      {kind === "cartoon" ? <CartoonGuide body={worked} href={href} /> : kind === "prose" || kind === "poetry" ? <LiteraryReadingGuide key={itemId} body={worked} href={href} example={kind === "poetry" ? poetryExample : proseExample} /> : kind === "infographic" ? <InfographicGuide body={worked} /> : kind === "advertisement" ? <AdvertisementGuide body={worked} href={href} /> : kind === "charity-appeal" ? <CharityAppealGuide body={worked} href={href} /> : kind === "drama" ? <DramaGuide body={worked} href={href} /> : kind === "nonfiction" ? <NonfictionGuide body={worked} href={href} /> : <Markdown>{worked}</Markdown>}
    </> : <>
      {kind === "cartoon" || kind === "poetry" || kind === "prose" || kind === "advertisement" || kind === "charity-appeal" || kind === "drama" || kind === "nonfiction" ? <TextTypeOverview body={overview} href={href} guideTitle={kind === "cartoon" ? "Cartoons" : kind === "poetry" ? "Poetry" : kind === "prose" ? "Prose fiction" : kind === "advertisement" ? "Advertisement" : kind === "drama" ? "Drama" : kind === "nonfiction" ? "Prose non-fiction" : "Charitable appeal"} exampleLabel={kind === "cartoon" ? "Andy Singer worked example" : kind === "poetry" ? "Up-Hill worked example" : kind === "prose" ? "Moon Tiger worked example" : kind === "advertisement" ? "FIJI Water worked example" : kind === "drama" ? "Bovell worked example" : kind === "nonfiction" ? "Fisher worked example" : "WWF worked example"} /> : <div className="text-type-overview"><Markdown>{overview}</Markdown></div>}
