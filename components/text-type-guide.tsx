...
export default function TextTypeGuide({ body, itemId, href, showExample, exampleChoice }: { body: string; itemId: string; href: string; showExample: boolean; exampleChoice?: "wwf" | "cycling" }) {
  ...
  return <>
    ...
    {showExample && worked ? <>
      ...
      {kind === "blog" ? <BlogGuide body={worked} href={href} /> : kind === "cartoon" ? <CartoonGuide body={worked} href={href} /> : kind === "prose" || kind === "poetry" ? <LiteraryReadingGuide key={itemId} body={worked} href={href} example={kind === "poetry" ? poetryExample : proseExample} /> : kind === "infographic" ? (isAdditional ? <CyclingGuide body={worked} href={href} /> : <InfographicGuide body={worked} />) : kind === "advertisement" ? <AdvertisementGuide body={worked} href={href} /> : kind === "charity-appeal" ? (isAdditional ? <CharityAppealGuide body={worked} href={href} /> : <ElephantAppealGuide body={worked} href={href} />) : kind === "drama" ? <DramaGuide body={worked} href={href} /> : kind === "nonfiction" ? <NonfictionGuide body={worked} href={href} /> : <Markdown>{worked}</Markdown>}
    </> : <>
      {kind === "blog" || kind === "cartoon" || kind === "poetry" || kind === "prose" || kind === "advertisement" || kind === "charity-appeal" || kind === "drama" || kind === "nonfiction" ? <TextTypeOverview body={overview} href={href} guideTitle={kind === "blog" ? "Blog post" : kind === "cartoon" ? "Cartoons" : kind === "poetry" ? "Poetry" : kind === "prose" ? "Prose fiction" : kind === "advertisement" ? "Advertisement" : kind === "drama" ? "Drama" : kind === "nonfiction" ? "Prose non-fiction" : "Charitable appeal"} exampleLabel={kind === "blog" ? "Emberton worked example" : kind === "cartoon" ? "Andy Singer worked example" : kind === "poetry" ? "Up-Hill worked example" : kind === "prose" ? "Moon Tiger worked example" : kind === "advertisement" ? "FIJI Water worked example" : kind === "drama" ? "Bovell worked example" : kind === "nonfiction" ? "Fisher worked example" : "Elephant Sanctuary worked example"} /> : <div className="text-type-overview"><Markdown>{overview}</Markdown></div>}
