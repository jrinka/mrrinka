...
export default function AdvertisementOverview({ body, href }: { body: string; href: string }) {
  const sections = splitGuideSections(body);
  return <div className="advertisement-overview-layout">
    <nav className="advertisement-contents" aria-label="Advertisement overview contents">
      ...
      <Link className="advertisement-contents-example" href={`${href}?view=example`}><span className="mono">PUT IT INTO PRACTICE</span>FIJI Water worked example →</Link>
    </nav>
