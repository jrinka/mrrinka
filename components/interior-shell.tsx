...
import { BookOpen, Library, CalendarDays, Sparkles, FilePenLine, Home, Menu, X, PanelLeftClose, PanelLeftOpen } from "lucide-react";
import type { CourseId } from "@/lib/schema";
const defaults = [{id:"language-literature",short:"IB Lang & Lit",side:"Language & Literature"},{id:"literature",short:"IB Literature",side:"Literature"},{id:"english-10",short:"English 10",side:"English 10"}];
export default function InteriorShell({children,courseId,courseLabels=defaults}:{children:React.ReactNode;courseId?:CourseId;courseLabels?:typeof defaults}) {
  ...
  const local = courseId ? [
    ...
    {href:`${base}/practice`,label:"Practice",icon:Sparkles},
  ] : [];
  ...
  return <div className={`interior-site ${collapsed ? "interior-collapsed" : ""}`}>
    ...
    <aside id="interior-navigation" className={`interior-sidebar ${mobileOpen ? "mobile-open" : ""}`} onKeyDown={event=>{if(event.key==="Escape"){setMobileOpen(false);mobileToggle.current?.focus();}}}>
      ...
      <label className="interior-course"><span className="mono">COURSES</span><select aria-label="Choose a course" value={courseId ?? ""} onChange={e=>{setMobileOpen(false);router.push(`/courses/${e.target.value}`);}}><option value="" disabled>Choose a course</option>{courseLabels.map(c=><option key={c.id} value={c.id}>{c.short}</option>)}</select></label>
