...
import { webpageExample as example } from "@/lib/webpage-example";
...
export default function WebpageGuide({ body, href }: { body: string; href: string }) {
  ...
  return <div className="advertisement-guide">
    ...
    <div className="advertisement-reading-workspace" ref={workspaceRef}>
      <aside className={`advertisement-viewer ${styles.source}`} ref={sourceRef} tabIndex={0} aria-label="Webpage source views">
        <div className="text-type-example-head"><span className="mono">SOURCE / REDWOODS TREEWALK</span><button type="button" onClick={() => dialogRef.current?.showModal()}>Enlarge source ↗</button></div>
      ...
      <div className="advertisement-analysis">
        ...
        {sections.map((section, index) => <section className="advertisement-reading-section" key={section.id} hidden={activeIndex !== index} aria-labelledby={section.id}>
          ...
          {section.id === "practice-and-transfer" && <WebpageNotes />}
        </section>)}
    ...
    <dialog ref={dialogRef} className={`advertisement-source-dialog ${styles.dialog}`} aria-labelledby="webpage-source-title" onClick={event => { if (event.target === dialogRef.current) dialogRef.current?.close(); }}>
      <div className="advertisement-dialog-heading"><h2 id="webpage-source-title">Redwoods Treewalk — source viewer</h2><button type="button" autoFocus onClick={() => dialogRef.current?.close()}>Close ×</button></div>
      <SourceControls viewIndex={viewIndex} onChange={setViewIndex} enlarged /><SourceFigure viewIndex={viewIndex} />
    </dialog>
  </div>;
}

function WebpageNotes() {
  const [values, setValues] = useState({ audience: "", evidence: "", analysis: "" });
  const [format, setFormat] = useState<ExportFormat>("txt");
  return <section className="infographic-notes" aria-label="Webpage analysis notebook">
    <h3>Try a connected reading</h3>
    {fields.map(field => <label className="field" key={field.key} htmlFor={`webpage-${field.key}`}>{field.label}<span className="hint" id={`webpage-${field.key}-hint`}>{field.hint}</span><textarea id={`webpage-${field.key}`} aria-describedby={`webpage-${field.key}-hint`} rows={field.key === "analysis" ? 6 : 3} maxLength={6000} value={values[field.key]} onChange={event => setValues(previous => ({ ...previous, [field.key]: event.target.value }))} /></label>)}
    ...
    <div className="tool-actions"><ExportFormatSelect value={format} onChange={setFormat} /><button className="button secondary" type="button" onClick={() => downloadRecord(`Webpage analysis notes\n\nSource: ${example.title}\nhttps://mrrinka.com${example.views[0].image}\nhttps://mrrinka.com${example.views[1].image}\n\nQuestion from the supplied paper: ${example.question}\n\n${fields.map(field => `${field.label}\n${values[field.key] || "(Not yet written)"}`).join("\n\n")}\n\nStudent-authored notes; no AI feedback.`, "redwoods-webpage-notes.txt", format)}>Export my notes</button></div>
    <details className="infographic-self-check"><summary>Review your paragraph</summary><ul><li>Have I connected specific wording and visual evidence?</li><li>Have I explained how the page helps visitors imagine or plan the experience?</li><li>Have I read the prices through their headings and categories?</li><li>Have I kept claims about interaction within what the source shows?</li></ul></details>
  </section>;
}
...
function SourceFigure({viewIndex}:{viewIndex:number}) {
 ...
 return <figure className={styles.figure}><Image src={view.image} alt={view.alt} width={view.width} height={view.height} unoptimized /><figcaption>{view.label} · Redwoods Treewalk. Supplied exam version; individual photographers unidentified. <a href={view.image} target="_blank" rel="noopener noreferrer">Full-size source image ↗</a> · <a href={example.url} target="_blank" rel="noopener noreferrer">Redwoods Treewalk ↗</a></figcaption></figure>;
}
