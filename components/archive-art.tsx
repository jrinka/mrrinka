...
export function GuidePlate({ kind }: { kind: "methods" | "lenses" | "practice" | "language" }) {
  const plate = kind === "methods" ? plates["language-literature"].assessment
    : kind === "lenses" ? plates.literature.hero
    : kind === "language" ? plates["language-literature"].resources : plates.literature.practice;
  return (
    <figure className="guide-plate">
      <Image src={plate.src} alt={plate.alt} fill
        sizes={kind === "language" ? "(max-width: 700px) 100vw, 45vw" : "(max-width: 600px) 160px, 240px"}
