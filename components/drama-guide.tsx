...
import { dramaExample as example, dramaReadingSections as reading, dramaNoteFields as fields, dramaPassages } from "@/lib/drama-example";
import styles from "./drama.module.css";

export default function DramaGuide({ body, href }: { body: string; href: string }) {
    ...
    <div className="advertisement-reading-workspace" ref={workspaceRef}>
      <aside className={`advertisement-viewer ${styles.source}`} ref={sourceRef} tabIndex={0} aria-label="Drama extract: Fran and Bob">
        ...
        <div className={styles.transcript}>
          {dramaPassages.map((passage, index) => <section className={styles.passage} key={passage.title} hidden={!wholeSource && note?.passage != null && note.passage !== index}>
            <h3>{passage.title}</h3>
      ...
      <div className="advertisement-analysis">
        ...
        {sections.map((section, index) => <section className="advertisement-reading-section" key={section.id} hidden={activeIndex !== index} aria-labelledby={section.id}>
          ...
          {section.id === "practice-and-transfer" && <DramaNotes />}
        </section>)}
